...
for TEST in test_params:
    ...
    # appliedError = amplitude_independent_errors_normal_noise(nfDataError, TEST[0])
    # appliedError = amplitude_single_pol_errors_normal_noise(nfDataError, TEST[0])
    # appliedError = phase_errors_correlated_phi_same(nfDataError, deviationFactor, TEST[0])
    # appliedError = phase_errors_correlated_theta_same(nfDataError, deviationFactor, TEST[0])
    # appliedError = phase_same_errors_uniform(nfDataError, TEST[0])
    ...
    # compare/dif plots
    plot_error_compare(selected_ffData_no_error, selected_ffData_error, theta_deg_center, f'Radiation Comparison of transform w/o error')
    plt.savefig(PATH_PREFIX + 'compare_amplitude_correlation_reverse.svg', bbox_inches='tight')

    plot_dif(selected_ffData_no_error, selected_ffData_error, theta_deg_center, f'Diff radiation of transform w/o error')
