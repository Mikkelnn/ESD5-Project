...
def parse_file(filename):
    ...
    # Extract parameters
    test_params_match = re.search(r'TEST_PARAMS: (\S+) (\S+)', data)
    ...
    # Extract HPBW values
    h_smooth = float(re.search(r'H-plane \(smoothed\) HPBW: ([\d.]+)', data).group(1))
    h_orig = float(re.search(r'H-plane \(original\) HPBW: ([\d.]+)', data).group(1))
    e_smooth = float(re.search(r'E-plane \(smoothed\) HPBW: ([\d.]+)', data).group(1))
    e_orig = float(re.search(r'E-plane \(original\) HPBW: ([\d.]+)', data).group(1))
