...
def calc_delta(m, mp, n):
    binomfunc = 0
    lowerbound = max(-m -mp,0)
    higherbound = min(n-mp, n-m)
    for sigma in range(lowerbound, higherbound + 1):
        i1 = (n+m)
        j1 = (n-mp-sigma)
        i2 = (n-m)
        j2 = (sigma)
        binomfunc += sci_sp.binom(i1, j1) * sci_sp.binom(i2, j2) * (-1)**(n-mp)
    part1 = (math.factorial(n+mp) * math.factorial(n-mp))
    part2 = (math.factorial(n+m) * math.factorial(n-m))
    delta = (np.sqrt(part1) / np.sqrt(part2)) * (1 / (2**mp)) * binomfunc
    return delta
...
def spherical_far_field_transform_megacook(nf_data, theta_f, phi_f, Δθ, Δφ, frequency_Hz, nf_meas_dist = 3.2, N = 3, M = 3):
    ...
    for n_idx, n in enumerate(n_range):
        ...
        for l in j_range:
            ...
            if (l - n) % 2 == 0:
                PI_lm = 2 / (1 - ((l - n)**2))
            K_mmu1 += PI_lm * bl_mmu1 #Equation 4.130
            K_mmu2 += PI_lm * bl_mmu2 #Equation 4.130
        for m_idx, m in enumerate(m_range):
            w_n_uA_const = ((2*m+1)/2) * 1j**(-n)
            for mp_idx, m_mark in enumerate(m_range):
                w_n_uA1 = calc_delta(m, -1, n) * calc_delta(m, m_mark, n) * K_mmu1 #Equation 4.132
                w_n_uA2 = calc_delta(m, 1, n) * calc_delta(m, m_mark, n) * K_mmu2 #Equation 4.132
            w_n_uA1 *= w_n_uA_const
            w_n_uA2 *= w_n_uA_const
            T2[mp_idx, m_idx] = ((w_n_uA2[n_idx, m_idx] * PHertzian[m_idx,0,0]) - (w_n_uA1[n_idx, m_idx] * PHertzian[n_idx, 0, 1])) / ((PHertzian[n_idx, 0, 0] * PHertzian[n_idx, 1, 1]) - (PHertzian[n_idx, 1, 0] * PHertzian[n_idx, 0, 1]))
            T1[mp_idx, m_idx] = ((-T2[mp_idx, m_idx]) * PHertzian[m_idx, 1, 0] + w_n_uA1[n_idx, m_idx]) / PHertzian[n_idx, 0, 0]
    print(T1)
