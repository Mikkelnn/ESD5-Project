...
def plot_dif(data, data2, theta_f_deg, figure_title):
    ...
    ax1 = fig.add_subplot(grid[0, 0])
    ax1.plot(theta_f_deg, abs(h_plane_data_smooth - h_plane_data_smooth2) , label=f'Dif plot (smoothed)', alpha=0.7)  
    ax1.plot(theta_f_deg, abs(h_plane_data_original - h_plane_data_original2) , label=f'Dif plot (not smoothed)', alpha=0.7)  
    ...
    ax2 = fig.add_subplot(grid[1, 0])
    ax2.plot(theta_f_deg, abs(e_plane_data_smooth - e_plane_data_smooth2) , label=f'Dif plot (smoothed)', alpha=0.7)  
    ax2.plot(theta_f_deg, abs(e_plane_data_original - e_plane_data_original2) , label=f'Dif plot (not smoothed)', alpha=0.7)  
...
def plot_error_compare(data, data2, theta_f_deg, figure_title):
    ...
    # ax1.plot(theta_f_deg, h_plane_data_original , label=f'Padding with last data', alpha=0.7)
    # ax1.plot(theta_f_deg, h_plane_data_original2, label=f'Padding with zeros', alpha=0.7)    
    ax1.set_title(f'{figure_title} H-plane Phi = {h_plane_plot_angle}')
    ax1.set_xlabel('Theta')
    ax1.grid()
    ax1.legend()
    ...
    # ax2.plot(theta_f_deg, e_plane_data_original, label=f'Padding with last data', alpha=0.7)
    # ax2.plot(theta_f_deg, e_plane_data_original2, label=f'Padding with zeros', alpha=0.7)    
    ax2.set_title(f'{figure_title} E-plane Phi = {e_plane_plot_angle}')
    ax2.set_xlabel('Theta')
    ax2.grid()
    ax2.legend()
...
def plot_polar_old(data, theta_f, phi_f, figure_title):
    # select data and roll back, roll ensures center of main lobe is at 0 deg
...
def plot_copolar_old(data, theta_f_deg, phi_f_deg, figure_title):
