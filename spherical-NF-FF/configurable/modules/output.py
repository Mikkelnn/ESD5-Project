...
def plot_dif(data, data2, theta_f_deg, figure_title):
    ...
    ax1 = fig.add_subplot(grid[0, 0])
    ax1.plot(theta_f_deg, abs(h_plane_data_smooth - h_plane_data_smooth2) / np.max(h_plane_data_smooth) , label=f'Dif plot (smoothed)', alpha=0.7)  
    ax1.plot(theta_f_deg, abs(h_plane_data_original - h_plane_data_original2) / np.max(h_plane_data_original) , label=f'Dif plot (not smoothed)', alpha=0.7)  
    ...
    ax2 = fig.add_subplot(grid[1, 0])
    ax2.plot(theta_f_deg, abs(e_plane_data_smooth - e_plane_data_smooth2) / np.max(e_plane_data_smooth) , label=f'Dif plot (smoothed)', alpha=0.7)  
    ax2.plot(theta_f_deg, abs(e_plane_data_original - e_plane_data_original2) / np.max(e_plane_data_original) , label=f'Dif plot (not smoothed)', alpha=0.7)  
