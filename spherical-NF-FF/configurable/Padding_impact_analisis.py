...
nfData_zero_pad = np.copy(nfData)
nfData_zero_pad, _, _ = pad_theta_zeros(nfData_zero_pad, theta_step_deg)
nfData_pad_last_val, theta_deg2, num_zero_nfData = pad_theta(nfData, theta_step_deg)
...
# pre-process nfData
nfData_sum_pad_last_val = HansenPreProcessing(nfData_pad_last_val)
nfData_sum_zero_pad = HansenPreProcessing(nfData_zero_pad)

# transform NF to FF
ffData_pad_last_val = spherical_far_field_transform_SNIFT(nfData_sum_pad_last_val, frequency_Hz, transform_from_dist_meters, transform_to_dist_meters)
ffData_zero_pad = spherical_far_field_transform_SNIFT(nfData_sum_zero_pad, frequency_Hz, transform_from_dist_meters, transform_to_dist_meters)

# post-process FF
ffData_pad_last_val_2D = sum_NF_poles_sqrt(ffData_pad_last_val)
ffData_zero_pad_2D = sum_NF_poles_sqrt(ffData_zero_pad)

# Normalize plots
ffData_pad_last_val_2D = ffData_pad_last_val_2D / np.max(np.abs(ffData_pad_last_val_2D))
ffData_zero_pad_2D = ffData_zero_pad_2D / np.max(np.abs(ffData_zero_pad_2D))

# Remove original zero padding
ffData_pad_last_val_2D = removeXFromEnd(ffData_pad_last_val_2D, int(num_zero_nfData))
ffData_zero_pad_2D = removeXFromEnd(ffData_zero_pad_2D, int(num_zero_nfData))
...
ffData_pad_last_val_2D = 20 * np.log10(ffData_pad_last_val_2D)
ffData_zero_pad_2D = 20 * np.log10(ffData_zero_pad_2D)

selected_data_pad_last_val = select_data_at_angle(ffData_pad_last_val_2D, phi_deg, phi_select_angle)
selected_data_zero_pad = select_data_at_angle(ffData_zero_pad_2D, phi_deg, phi_select_angle)


##############################################################################################################
# 5. Output FF - plot or write to file
##############################################################################################################
print("NF padded with last value:")
calculate_print_hpbw(selected_data_pad_last_val, theta_deg_center)

print("NF padded with zeros:")
calculate_print_hpbw(selected_data_zero_pad, theta_deg_center)

plot_error_compare(selected_data_pad_last_val, selected_data_zero_pad, theta_deg_center, f'Comparison plot padding types')
plot_dif(selected_data_pad_last_val, selected_data_zero_pad, theta_deg_center, f'Difference plot padding types')
