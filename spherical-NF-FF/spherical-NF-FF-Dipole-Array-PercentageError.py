...
theta = np.linspace(0, 2 * np.pi, num_samples_theta)  # Polar angle
phi = np.linspace(0, np.pi, num_samples_phi)  # Azimuthal angle
...
for pos in dipole_positions:
    for i in range(num_samples_theta):
        for j in range(num_samples_phi):
            ...
            observation_x = random.uniform(radius_nf*errorPercent, -radius_nf*errorPercent) + radius_nf * np.sin(theta_grid[i, j]) * np.cos(phi_grid[i, j])  # x-coordinate
            observation_y = random.uniform(radius_nf*errorPercent, -radius_nf*errorPercent) + radius_nf * np.sin(theta_grid[i, j]) * np.sin(phi_grid[i, j])  # y-coordinate
            observation_z = random.uniform(radius_nf*errorPercent, -radius_nf*errorPercent) + radius_nf * np.cos(theta_grid[i, j])  # z-coordinate
...
ax3.set_title('E_theta (Near Field)')
...
ax1.plot(theta, E_theta_avg, label='E_theta (Near Field)', color='blue', alpha=0.7)
ax1.plot(theta, E_phi_avg, label='E_phi (Near Field)', color='red', alpha=0.7)
ax1.set_title('Normalized Near-field Pattern')
ax1.set_ylim(0, 1.1)  # Set y-limit for better visibility
ax1.legend()

# Set parameters for far-field computation
max_l = 10  # Maximum order of spherical harmonics
theta_f = np.linspace(0, 2*np.pi, num_samples_theta)  # Far-field theta (0 to π)
phi_f = np.linspace(0, np.pi, num_samples_phi)  # Far-field phi (0 to 2π) azimuth
...
def far_field_pattern(a_lm, theta_f, phi_f, max_l):
    ...
    return np.real(E_far_phi), np.real(E_far_theta)

# Compute far-field patterns
a_lm = compute_far_field(nf_data, max_l)
E_phi_far, E_theta_far = far_field_pattern(a_lm, theta_f, phi_f, max_l)

# Normalize the far-field electric field magnitudes
E_theta_far_mod = np.abs(E_theta_far) #Reactive power bounces back, hence we are only interested in real power.
...
# Far field plot
ax2 = plt.subplot(2, 2, 3, projection='polar')

# Plot E_theta and E_phi for the far field
ax2.plot(theta_f + np.pi/2, E_theta_far_mod[0, :], label='E_theta (Far Field, θ=0)', alpha=0.7) #The addition of pi/2 means that the array can be imagined as going from left to right, and seen from above.
ax2.plot(theta_f + np.pi/2, E_phi_far_mod[0, :], label='E_phi (Far Field, θ=0)', alpha=0.7)
ax2.plot(theta_f + np.pi/2, E_theta_far_mod[-1, :], label='E_theta (Far Field, θ=π)', alpha=0.7)
ax2.plot(theta_f + np.pi/2, E_phi_far_mod[-1, :], label='E_phi (Far Field, θ=π)', alpha=0.7)

ax2.set_title('Normalized Far-field Pattern')
ax2.legend()
'''
# Add a 3D plot for far field
ax4 = plt.subplot(2, 2, 4, projection='3d')

# Convert far field spherical to Cartesian coordinates for 3D plotting
x_ff = radius_ff * np.outer(np.sin(theta_f), np.cos(phi_f))  # X-coordinates
y_ff = radius_ff * np.outer(np.sin(theta_f), np.sin(phi_f))  # Y-coordinates
z_ff = radius_ff * np.outer(np.cos(theta_f), np.ones(num_samples_phi))  # Z-coordinates

# Color mapping based on E_theta magnitude in far field
E_theta_far_flat = E_theta_far_mod.flatten()  # Flatten the surface for color mapping
norm_ff = plt.Normalize(E_theta_far_flat.min(), E_theta_far_flat.max())
colors_ff = plt.cm.viridis(norm_ff(E_theta_far_flat)).reshape(E_theta_far_mod.shape + (4,))  # Reshape colors

# Create surface plot for far field
surf_ff = ax4.plot_surface(x_ff, y_ff, z_ff, facecolors=colors_ff, rstride=1, cstride=1, alpha=0.7, linewidth=0)

ax4.set_title('E_theta (Far Field)')
